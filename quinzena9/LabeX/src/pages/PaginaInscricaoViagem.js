...
import { Botao, CorpoPagina, Formulario } from "../components/styled";
...
export default function PaginaInscricaoViagem() {
  ...
  const onClickInscricao = (event) => {
    ...
    axios.post(`${api_url}/trips/${pathParams.id}/apply`, body)
      .then(() => {
        limpaFormulario()
      })
  ...
  return (
    <CorpoPagina>
      <Header />
      <h1>Inscreva-se para a viagem "{pathParams.titulo}"</h1>
      <Formulario onSubmit={onClickInscricao}>
        <input
          name='name'
          type='text'
          value={form.name}
